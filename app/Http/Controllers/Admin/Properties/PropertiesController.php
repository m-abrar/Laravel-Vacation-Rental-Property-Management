...
class PropertiesController extends Controller
{
    ...
    public function create()
    {
        ...
        $states       = ModelLocations::where('type', 'state')->orderBy('is_active', 1)->orderBy('display_order', 'asc')->get();
        ...
        $owners       = User::where('is_active', '1')->orderBy('firstname', 'asc')->get();
        ...
        foreach ($lineitemsdefault as $lineitem) {
            $lineitem->value_default = $lineitem->value;
            $lineitem->value = null;
            array_push($lineitems, $lineitem);
        } 
        $settings     = Settings::find(1);
        return view('admin.properties.create')->with('settings', $settings)->with('categories', $categories)->with('classes', $classes)->with('states', $states)->with('housekeepers', $housekeepers)->with('vendors', $vendors)->with('owners', $owners)->with('rates', $rates)->with('amenities', $amenities)->with('features', $features)->with('lineitems', $lineitems);
    }
    ...
    public function edit($id)
    {
        ...
        $lineitems          = array();
        foreach ($lineitemsdefault as $lineitem) {
            $customlineitems    = PropertiesLineItems::where('property_id', $id)->where('lineitem_id', $lineitem->id)->get();
            $lineitem->value_default = $lineitem->value;
        ...
        $settings     = Settings::find(1);
        return view('admin.properties.edit')->with('settings', $settings)
        ...
        ->with('amenities', $amenities_final)->with('features', $features_final)->with('lineitems', $lineitems);
    }
    //The common part of both insert/update code has been put in a single function place called save().
    public function save($request, $property)
    {
        ...
        $property->minimum_stay_nights            = $request->input('minimum_stay_nights');/*upgrade - 12/10/2016 - 
        minimum_nights*/
        ...
        if (!empty($property_id)) {
            ...
            $PLineItems = PropertiesLineItems::where('property_id', $property_id);
            $PLineItems->delete();
            $lineitems = LineItems::where('is_active', '1')->orderBy('display_order', 'asc')->get();
            foreach ($lineitems as $lineitem) {
                $propertyLineItem              = new PropertiesLineItems();
                $propertyLineItem->property_id = $property_id;
                $propertyLineItem->lineitem_id  = $lineitem->id;
                $propertyLineItem->value       = $request->get('lineitem_value_' . $lineitem->id);
                $propertyLineItem->save();
            } //$features as $feature
            @$success .= 'LineItems have been added.<br/>';
